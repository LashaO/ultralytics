...
def verify_image(args):
    ...
    try:
        with rasterio.open(im_file) as src:
            ...
            assert count >= 1, f"No bands found in image"
            if src.driver != 'GTiff':
                raise ValueError(f"Unsupported format: {src.driver}")
            nf = 1
    except (AssertionError, RasterioIOError, Exception) as e:
        nc = 1
        msg = f"{prefix}{im_file}: x ignoring dd corrupt image/label: {e}"
...
#     except Exception as e:
#         nc = 1
#         msg = f"{prefix}{im_file}: ignoring corrupt ss image/label: {e}"
...
def verify_image_label(args):
    """Verify one image-label pair with TIFF multi-band support."""
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls = args
    nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, "", [], None
    ...
    try:
        ...
        if suffix == '.tif' or suffix == '.tiff':
            ...
            with rasterio.open(im_file) as src:
                shape = (src.height, src.width)
                assert shape[0] > 9 and shape[1] > 9, f"image size {shape} <10 pixels"
                assert src.count >= 1, "TIFF file has no bands"
            nf = 1
        else:
            ...
            shape = (shape[1], shape[0])  # to (height, width)
            assert shape[0] > 9 and shape[1] > 9, f"image size {shape} <10 pixels"
            assert im.format.lower() in IMG_FORMATS, f"invalid image format {im.format}. {FORMATS_HELP_MSG}"
            if im.format.lower() in {"jpg", "jpeg"}:
                with open(im_file, "rb") as f:
        ...
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt bb image/label: {e}"
        return [None, None, None, None, None, nm, nf, ne, nc, msg]
